@if ($article->id_etat != 2 && $article->quantite_disponible > 0)
    <div x-data='{openArticleModal: false}'
        class="inline lg:w-64 mx-2 shrink-0 overflow-hidden whitespace-nowrap rounded-[16px] bg-white shadow-md hover:shadow-lg">
        ...
        <div class="h-fit flex flex-row p-4">
            <div class="flex-col w-[70%] h-full">
                ...
                <p class="text-gray-600">{{ number_format($article->prix, 2, ',', ' ') }} $</p>
                ...
                <div class="mt-8">
                    ...
                    @else
                        ...
                        @endif
                        <button
                            class="w-full add-to-cart overflow-hidden whitespace-nowrap border-darkGrey border rounded-[24px] h-[32px] text-darkGrey font-bold"
                            x-data="{ ajoute: {{ $ajoute ? 'true' : 'false' }} }" @click="addToCart({{ $article->id_article }}); ajoute = !ajoute"
                            :class="ajoute ? 'added' : ''">
                            <div value="{{ $article->id_article }}" name="id_article" class="default m-auto">
                                Ajouter au panier
                            </div>
                ...
                <div class="flex flex-col items-center">
                    @auth
                        <div x-cloak x-data="{ liked: {{ json_encode($article->isLikedByUser(Auth::id()) != '' ? true : false) }} }"
                            ...
                            <!-- Heart SVG Icon -->
                            <svg width="32" height="32" viewBox="12 18 24 24"
                                class="relative z-20 transition-all duration-200">
                    ...
                    @else
                        <div x-cloak @click="$dispatch('open-login-modal')" class="relative cursor-pointer w-auto h-auto">
                    ...
                    <a href="{{ route('kiosque', ['idUser' => $article->artiste->id_user]) }}">
                        <img src="{{ asset($article->getArtiste->path_photo_profil ?? 'img/artistePFP/default_artiste.png') }}"
                            alt="{{ $article->getArtiste->nom_artiste }}" class="rounded-full w-[48px] h-[48px]"></a>
                </div>
            </div>
        </div>
    </div>
