<div x-cloak x-data="{ openArticleSettings: false }" @close-login-modal.window="openArticleSettings = false">
    ...
    <div x-show="openArticleSettings"
        ...
        <div class="bg-white p-6 rounded-[12px] shadow-lg w-full max-w-[1200px] relative">
            ...
            <div class="flex justify-between items-center">
                <div class="flex items-center">
                    <button id="ajouterArticle"
                        ...
                        Ajouter un
                        article </button>
                    <button id="tousMesArticles" class="font-bold text-[16px] border-[2px] border-darkGrey p-[8px] rounded-t-[12px]"> Tous mes
                        articles </button>
                    <button id="articlesEnVedette" class="font-bold text-[16px] border-[2px] border-darkGrey p-[8px] rounded-t-[12px]"> Articles en
                        vedette </button>
                    <button id="personnalisation" class="font-bold text-[16px] border-[2px] border-darkGrey p-[8px] rounded-t-[12px]">
            ...
            <div>
                ...
                <form method="POST" action="{{ route('addArticle') }}" class="w-full rounded-r-[12px] rounded-b-[12px] bg-darkGrey">
                    @csrf

                 <!-- nom Article -->
                <div class="mb-4">
                    <x-input-label for="nomArticle"/>
                    <x-text-input id="nomArticle" class="block w-[250px]" type="text" name="nomArticle"/>
                    <x-input-error :messages="$errors->get('nomArticle')" class="mt-2" />
                </div>
