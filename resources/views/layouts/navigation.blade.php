        ...
        <div class="flex justify-end w-[225px] items-center mr-[16px]">
            ...
            @if (Auth::user() != null)
                @if (Auth::user()->moderateur)
                    <span class="relative group -translate-y-3 -translate-x-3">
    ...
    translate-y-2 top-full left-1/2 transform -translate-x-1/2">
                ...
                @if (Auth::user()->artiste)
                    @if (Auth::user()->artiste->actif == 1)
                        <span class="relative group -translate-x-3">
                            <a href="{{ route('tousMesArticles') }}" class="ml-[15px] flex items-center">
                                <svg width="38" height="38" viewBox="0 0 78 78" fill="none"
                                    ...
                                    <path
                                        ...
                                        stroke="#444444" stroke-width="4.5" stroke-linecap="round"
            ...
            @auth
                @if (!Auth::user()->moderateur && !Auth::user()->artiste)
                    <span class="relative group -translate-y-3 -translate-x-3">
                        ...
                        <!-- Tooltip content -->
                        <div
                            class="absolute bg-hoverBeige text-md rounded-md p-2 shadow-lg w-fit text-nowrap
                                        invisible opacity-0 group-hover:visible group-hover:opacity-100 transition duration-200 ease-in-out
                                        translate-y-2 top-full left-1/2 transform -translate-x-1/2">
                            Devenir vendeur
                        </div>
            ...
            <span class="relative group -translate-y-3">
                <a class="ml-[15px] relative" href="{{ route('panier') }}">
                    @if ($basketCount > 0)
                        <p id="basketCount"
                            class="mx-auto bg-red-500 text-white text-sm text-center rounded w-fit absolute -right-2 top-4 {{ $basketCount > 9 ? 'px-1' : 'px-1.5' }}">
                            {{ $basketCount }}</p>
                    @endif
