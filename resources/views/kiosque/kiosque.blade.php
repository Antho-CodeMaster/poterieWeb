                    ...
                    @if ($article->etat->etat == 'Visible client' && $article->quantite_disponible > 0 && $article->is_en_vedette == 1)
                        <div class="w-[300px] m-[16px] overflow-hidden whitespace-nowrap">
                            ...
                            <div class="flex justify-between items-center my-[10px]">
                                <div class="w-[80%]">
                                    <p class="article">{{ $article->nom }} ppppppppppppppp</p>

                                    <p id="prix" class="">{{ $article->prix }}$</p>
                                </div>
                                ...
                                @else
                                    <svg class="w-10 h-10 text-gray-800 dark:text-white mb-[2px]"
                            ...
                            <form action="{{route('decouverte')}}" method="GET">
                                <button type="submit" value="add" name="ajouterPanier" class="border-darkGrey border rounded-[24px] w-[100%] h-[40px] article">Ajouter au panier</button>
                            </form>
                        </div>
                    @endif
                @endforeach
